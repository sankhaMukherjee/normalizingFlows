...
In :numref:`banana` notice that we don't yet know the distribution of the banana distribution, and hence we dont know how to 
...
:math:`q_{\mathbf z}(\mathbf z) = q_{\mathbf y}(\mathbf y) \left|  \det \nabla_{\mathbf y}f(\mathbf y)  \right|^{-1}`. 
See part 5 of the calculations above.

.. figure:: https://raw.githubusercontent.com/sankhaMukherjee/normalizingFlows/master/results/img/0001_banana_color.png
       :align: center
       :name: banana_color

       The mapping of the points sampled in the gaussian distribution to the banana distribution colored
       with the knowledge of the probablity density at each point. 

In :numref:`banana_color`, the colors of each of the points have been updated with the color corresponding to the PDF
